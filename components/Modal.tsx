...
function Modal() {
    ...
    <MuiModal open={showModal} onClose={handleClose} className="fixed !top-7 left-0 right-0 z-50 mx-auto w-full max-w-5xl overflow-hidden 
        ...
        <Toaster position='bottom-center'/>
            <button onClick={handleClose} className="modalButton absolute right-5 top-5 !z-40 border-none bg-[#181818] hover:bg-[#181818]/80">
                <XIcon className='h-6 w-6'/>
            </button>

            <div className='relative pt-[56.25%]'>
                <ReactPlayer
                    url={`https://www.youtube.com/watch?v=${trailer}` || 'https://www.youtube.com/watch?v=dQw4w9WgXcQ'}
                    width="100%"
                    height="100%"
                ...
                <div className='absolute bottom-10 flex w-full items-center justify-between px-10'>
                    <div className='flex space-x-2'>
                        <button className='flex items-center gap-x-2 rounded bg-white font-bold text-black px-8 text-xl transition hover:bg-[#e6e6e6]'>
                        ...
                        <button className='modalButton' onClick={() => console.log("teste")}> 
                            <ThumbUpIcon className='h-7 w-7'/> 
                        </button>
                    </div>
                    <button className='modalButton flex items-center gap-x-2' onClick={() => setMuted(!muted)}>
                            {!muted && <MdOutlineVolumeUp className='h-7 w-7 text-white'/> || <MdOutlineVolumeOff className='h-7 w-7 text-white'/>}
                    </button>
                </div>
            </div>
